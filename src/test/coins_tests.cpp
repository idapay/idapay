    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        // Do a random modification.
        {
            ...
            if (insecure_rand() % 5 == 0 || coin.IsSpent()) {
                Coin idapay;
                idapay.out.nValue = insecure_rand();
                idapay.nHeight = 1;
                if (insecure_rand() % 16 == 0 && coin.IsSpent()) {
                    idapay.out.scriptPubKey.assign(1 + (insecure_rand() & 0x3F), OP_RETURN);
                    BOOST_CHECK(idapay.out.scriptPubKey.IsUnspendable());
                    added_an_unspendable_entry = true;
                } else {
                    idapay.out.scriptPubKey.assign(insecure_rand() & 0x3F, 0); // Random sizes so we can test memory usage accounting
                    (coin.IsSpent() ? added_an_entry : updated_an_entry) = true;
                    coin = idapay;
                }
                stack.back()->AddCoin(COutPoint(txid, 0), std::move(idapay), !coin.IsSpent() || insecure_rand() & 1);
            } else {
