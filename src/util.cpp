...
const char * const BITCOIN_CONF_FILENAME = "idapay.conf";
const char * const BITCOIN_PID_FILENAME = "idapayd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "idapay" is a composite category enabling all IDAPay-related debug output
            if(ptrCategory->count(string("idapay"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IDAPayCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IDAPayCore
    // Mac: ~/Library/Application Support/IDAPayCore
    // Unix: ~/.idapaycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IDAPayCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/IDAPayCore";
#else
    // Unix
    return pathRet / ".idapaycore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty idapay.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override idapay.conf
