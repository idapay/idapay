...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("IDAPay Core idapay-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  idapay-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded idapay transaction") + "\n" +
              "  idapay-tx [options] -create [commands]   " + _("Create hex-encoded idapay transaction") + "\n" +
              "\n";
    ...
    UniValue prevtxsObj = registers["prevtxs"];
    {
        for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
...

            {
                ...
                idapay.out.nValue = 0; // we don't know the actual output value
                idapay.nHeight = 1;
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded idapay transaction
